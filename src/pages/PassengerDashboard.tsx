...
export const PassengerDashboard: React.FC = () => {
  ...
  useEffect(() => {
    ...
    // Realtime: unique channel name to avoid stale subscriptions
    const channelName = `vehicles-live-${Date.now()}`;
    const sub = supabase
      .channel(channelName)
      .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'vehicles' }, () => fetchVehiclesSilent())
      .on('postgres_changes', { event: 'UPDATE', schema: 'public', table: 'vehicles' }, () => fetchVehiclesSilent())
      .subscribe();

    // Polling fallback every 20s for reliability
    const poll = setInterval(() => fetchVehiclesSilent(), 20000);
  ...
  const loadVehicles = async (showSpinner: boolean) => {
  ...
  const fetchVehicles = () => loadVehicles(true);
  const fetchVehiclesSilent = () => loadVehicles(false);
            ...
            {bookingSuccess ? (
              ...
              <div className="p-4">
                ...
                <div className="flex gap-3">
                  <button
                    onClick={() => { setShowPayment(false); setShowPickup(true); setBookingError(null); }}
