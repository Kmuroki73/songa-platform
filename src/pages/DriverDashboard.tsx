...
import {
  ...
  Navigation, Phone, X, Map, Camera, Trash2, Sparkles, User,
} from 'lucide-react';
...
async function uploadVehicleImage(file: File, driverId: string): Promise<{ url: string | null; error: string | null }> {
...
export const DriverDashboard: React.FC = () => {
  ...
  const handleAddVehicle = async (e: React.FormEvent) => {
    ...
    setAddError(null);
    if (!profile?.id) {
      setAddError('You must be logged in as a driver to add a trip.');
      return;
    }
    setUploading(true);
    ...
    let photos: string[] = [];
    if (imageFile) {
      const { url, error: uploadErr } = await uploadVehicleImage(imageFile, profile.id);
      if (uploadErr) {
        setAddError(uploadErr);
        setUploading(false);
        return;
      }
      if (url) photos = [url];
    ...
    setUploading(false);
    if (error) {
      setAddError(error.message || 'Failed to add trip. Please try again.');
    } else {
  ...
  const fetchVehiclesSilent = async () => {
    const { data } = await supabase
      .from('vehicles')
      .select('*')
      .eq('driver_id', profile?.id)
        ...
        <div className="max-w-3xl mx-auto px-4 sm:px-6 flex justify-between items-center py-3.5">
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              className="flex items-center gap-1.5 px-3 py-2 text-sm text-gray-500 hover:text-gray-900 hover:bg-gray-100 rounded-lg transition-colors"
            >
          ...
          {activeVehicles.length === 0 ? (
            ...
            <div className="space-y-3">
              {activeVehicles.map((vehicle) => {
                const totalSeats = vehicle.seat_count || PASSENGER_SEATS[vehicle.vehicle_type ?? ''] || 14;
        ...
        <div className="fixed inset-0 bg-black/60 flex items-end sm:items-center justify-center p-0 sm:p-4 z-50">
          <div className="bg-white w-full sm:rounded-2xl sm:max-w-md max-h-[92vh] overflow-y-auto rounded-t-2xl">
            <div className="flex justify-between items-center p-4 border-b border-gray-100">
              ...
              <button
                onClick={() => { setShowAddVehicle(false); setAddError(null); }}
